...
public class AdminController {
	...
	public String showNews(Model model, Integer page) throws Exception {

		List<ArticleWithBLOBs> list = null;
		//页码对象
		PagingVO pagingVO = new PagingVO();
		//设置总页数
		pagingVO.setTotalCount((int)newsService.getCountArticle());
		if (page == null || page == 0) {
			pagingVO.setToPageNo(1);
			list = newsService.findByPaging(1);
		} else {
			pagingVO.setToPageNo(page);
			list = newsService.findByPaging(page);
		}
	...
	public String addNewsUI(Model model) throws Exception {
	...
	public String addNews(ArticleWithBLOBs news, Model model)
			throws Exception {

		Boolean result = newsService.save(news);

		if (!result) {
			model.addAttribute("message", "新闻ID重复");
			return "error";
		}
	...
	public String editNewsUI(Integer id, Model model) throws Exception {
		if (id == null) {
			 //加入没有带文章id就进来的话就返回学生显示页面
			return "redirect:/admin/showNews";
		}
		ArticleWithBLOBs news = newsService.findById(id);
		if (news == null) {
			throw new CustomException("未找到该新闻");
		}
		List<College> list = collegeService.finAll();

		model.addAttribute("collegeList", list);
		model.addAttribute("news", news);    //wuruofeng
	...
	public String editNews(ArticleWithBLOBs news) throws Exception {

		newsService.updataById(news.getArcid(), news);
	...
	private String removeNews(Integer id) throws Exception {
		if (id == null) {
	...
	private String selectNews(long findByName, Model model)
			throws Exception {

		List<ArticleWithBLOBs> list = newsService.findByName(findByName);
